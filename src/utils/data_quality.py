...
def schema_check(df, df2):
    """
        Checks if the new data fetched, has the same schema
        as the data we have stored.

        :param df: the dataframe we get from the data stored in bronze layer
        :param df2: the dataframe created from newly fetched data
        :return: None
    """
...
def quality_assurance_clickstream(df):
    """
        Performs basic QA tests on clickstream dataframe. This includes
        - Missing value checks
        - Duplicate records checks
        - Data type checks
        - Unique value checks
        - Date inconsistencies checks

        :param df: clickstream dataframe we want to perform QA checks on
        :return: None
    """
...
def quality_assurance_transactions(df):
    """
        Performs basic QA tests on transactions dataframe. This includes
        - Missing value checks
        - Duplicate records checks
        - Invalid data checks
        - null value counts

        :param df: transactions dataframe we want to perform QA checks on
        :return: None
    """
...
def fetch_validate_data(date, api_type: str):
    """
        Fetches validation data for a particular date from given API type

        :param date: the date we want the validation data for
...
def validate_table(original_df):
    """
        Performs validation checks on given dataframe by comparing validation data
        and data in given dataframe

        :param original_df: dataframe we want to perform validation checks on
        :return: None
    """
